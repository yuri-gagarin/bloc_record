module BlocRecord
	module Utility
		...
		def underscore(camel_cased_word)
			#replace double colons with slash
			string = camel_cased_word.gsub(/::/, '/')
			#insert underscore between any all-caps prefixes
			string.gsub!(/([A-Z]+)([A-Z][a-z])/, '\1_\2')
			#insert underscore between any camelcased words
